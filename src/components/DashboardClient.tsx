...
import { 
  ...
  fetchDocumentDownloadUrlAction,
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-20 flex flex-col justify-between selection:bg-teal-600 selection:text-white font-sans transition-colors">
      <div>
            ...
            <div className="relative w-4/5 max-w-xs bg-white dark:bg-slate-900 text-slate-900 dark:text-white h-full shadow-2xl z-10 flex flex-col justify-between p-6 border-r border-slate-200 dark:border-slate-800 overflow-y-auto">
              <div className="space-y-6">
                <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-800">
                  ...
                  <button onClick={() => setIsMobileMenuOpen(false)} className="p-1.5 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 cursor-pointer">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                  <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Theme Appearance</span>
                  <ThemeToggleButton />
                </div>
                ...
                <nav className="flex flex-col space-y-1.5 pt-2">
                  <button 
                    ...
                    className={`flex items-center space-x-3.5 py-3 px-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors ${activeTab === 'wealth' ? 'bg-teal-700 text-white shadow-sm' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center space-x-3.5 py-3 px-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors ${activeTab === 'liabilities' ? 'bg-teal-700 text-white shadow-sm' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center space-x-3.5 py-3 px-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors ${activeTab === 'retirement' ? 'bg-teal-700 text-white shadow-sm' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center space-x-3.5 py-3 px-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors ${activeTab === 'directives' ? 'bg-teal-700 text-white shadow-sm' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center space-x-3.5 py-3 px-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-colors ${activeTab === 'feed' ? 'bg-teal-700 text-white shadow-sm' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}
                  >
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-2.5">
                <Link href="/profile" onClick={() => setIsMobileMenuOpen(false)} className="w-full flex items-center justify-between px-3.5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-semibold rounded-xl border border-slate-200 dark:border-slate-700 transition">
                  <span className="flex items-center gap-2"><Settings className="w-4 h-4 text-slate-500" /> Household Settings</span>
                </Link>
                {session.user.role === 'SUPER_ADMIN' && (
                  <Link href="/admin" onClick={() => setIsMobileMenuOpen(false)} className="w-full flex items-center justify-between px-3.5 py-2.5 bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100 dark:hover:bg-amber-900/40 text-amber-900 dark:text-amber-200 text-sm font-semibold rounded-xl border border-amber-200 dark:border-amber-800 transition">
                    <span className="flex items-center gap-2"><Shield className="w-4 h-4 text-amber-600 dark:text-amber-400" /> Admin Portal</span>
                  </Link>
                )}
                <form action={logoutAction} className="pt-1">
                  <button type="submit" className="w-full flex items-center justify-center gap-2 px-3.5 py-2.5 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 text-sm font-semibold rounded-xl border border-rose-200 dark:border-rose-900 hover:bg-rose-100 dark:hover:bg-rose-900/40 cursor-pointer transition">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 space-y-6">
          <div className="hidden md:flex bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-1.5 rounded-2xl items-center gap-2 overflow-x-auto shadow-sm">
    ...
    >
      {isDark ? (
        <>
          ...
          <span className="inline md:hidden">Light Mode</span>
        </>
      ) : (
        <>
          <Moon className="w-4 h-4 text-slate-600" />
          <span className="inline md:hidden">Dark Mode</span>
        </>
      )}
    </button>
  );
  ...
  return (
    <div className="space-y-4">
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200/85 dark:border-slate-800 sticky top-0 z-40 px-4 md:px-8 py-3.5 shadow-sm transition-colors">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <button 
              onClick={onOpenMenu} 
              className="md:hidden p-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-xl cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition"
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <button onClick={onOpenAiReader} className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white border border-slate-800 dark:border-slate-700 font-semibold text-sm rounded-xl transition cursor-pointer shadow-sm">
              ...
              <button 
                ...
                className="p-2 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl border border-slate-200 dark:border-slate-700 transition cursor-pointer disabled:opacity-50 shadow-sm"
              >
              ...
              {session.user.role === 'SUPER_ADMIN' && (
                <Link href="/admin" title="Admin Portal" className="p-2 bg-amber-50 dark:bg-amber-950/50 hover:bg-amber-100 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-200 rounded-xl border border-amber-200 dark:border-amber-800 transition cursor-pointer shadow-sm">
                  <Shield className="w-4 h-4" />
                </Link>
              )}
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex items-center justify-between">
          <div className="min-w-0">
            ...
            <div className="text-xl font-black font-mono text-teal-700 dark:text-teal-400 truncate">
              {Math.round(totalNetWorth).toLocaleString()} <span className="text-xs font-sans font-normal text-teal-600 dark:text-teal-500">{baseCurrency}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button 
              ...
              className="p-2.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer disabled:opacity-50 shadow-sm"
            >
              <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} />
            </button>
          </div>
      ...
      <div className="hidden md:block max-w-7xl mx-auto px-4 md:px-8 pt-2">
        <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 w-full lg:w-auto flex-1 max-w-4xl">
            {sortedCategories.map(([cat, val]) => (
              <div key={cat} className="bg-slate-50/70 dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 px-4 py-3 rounded-xl text-xs shadow-sm min-w-0">
                ...
                <span className={`font-mono font-bold text-sm block truncate mt-0.5 ${val < 0 ? 'text-rose-700 dark:text-rose-400' : 'text-slate-900 dark:text-white'}`}>
                  {Math.round(val).toLocaleString()} <span className="text-[11px] font-sans font-normal text-slate-500 dark:text-slate-400">{baseCurrency}</span>
                </span>
              </div>
            ))}
          </div>
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
      ...
      <div className="space-y-3">
        {ssnAssets.concat(pensionAssets, ppfAssets).map((asset) => {
          ...
          return (
            <div key={asset.id} className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
              <div className="w-full md:w-3/4">
                ...
                {isEditing(asset.id) ? (
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-xl">{getInstruction(asset)}</div>
                )}
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-4 py-2.5 rounded-xl text-left md:text-right shadow-sm">
                  ...
                  {isEditing(asset.id) ? (
                    <div className="flex items-center gap-1 mt-1">
                      <input
                        ...
                        className="w-24 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-1.5 py-0.5 text-xs font-mono text-slate-900 dark:text-white font-bold focus:outline-none"
                      />
                ...
                <button onClick={() => setEditing(asset.id, !isEditing(asset.id))} className="p-2 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-xl cursor-pointer shadow-sm transition">
            ...
            <div key={doc.id} className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex items-center justify-between gap-3 shadow-sm">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button 
                  ...
                  className="px-3 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-xs font-semibold transition-colors border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer disabled:opacity-50"
                >
                ...
                <button 
                  ...
                  className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg border border-slate-200 dark:border-slate-700 transition cursor-pointer"
                >
                  <Trash2 className="w-4 h-4" />
                </button>
              </div>
...
function AccountInstructionsHub({ assets }: { assets: any[] }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
      <div className="flex items-center gap-2 pb-3 border-b border-slate-200 dark:border-slate-800">
        <Shield className="w-5 h-5 text-slate-500 dark:text-slate-400" />
        <h3 className="text-sm font-bold text-slate-900 dark:text-white uppercase">Institution &amp; Account-Level Family Directives</h3>
      </div>
      <p className="text-sm text-slate-600 dark:text-slate-400">Write overarching login protocols, broker contact details, and succession steps for entire accounts.</p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
          ...
          <div className="space-y-1.5 max-h-48 overflow-y-auto">
            {uniqueAccounts.map((acct) => (
              <button
                ...
                className={`w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-mono transition-colors cursor-pointer border ${selectedAccount === acct ? 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white font-bold shadow-sm' : 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
              >
                {acct}
              </button>
            ))}
        ...
        <div className="md:col-span-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col justify-between gap-3 shadow-sm">
          {selectedAccount ? (
            <>
              <div>
                <div className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-1">Directives for: {selectedAccount}</div>
                <textarea
                  value={editingNote}
                  onChange={(e) => setEditingNote(e.target.value)}
                  placeholder="Enter succession notes, broker estate desk info, or multi-stock transfer instructions..."
                  rows={4}
                  className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-3 text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:border-slate-400 resize-none shadow-sm"
                />
              </div>
            ...
            <div className="flex items-center justify-center h-full text-xs text-slate-400 dark:text-slate-500 py-8">Select an account from the left to view or edit master family instructions.</div>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-4 flex flex-col gap-3 shadow-sm">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-xs">
        <select value={usr} onChange={(e) => setUsr(e.target.value)} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded p-1.5 text-slate-900 dark:text-white shadow-sm">{members.map(m => <option key={m.id} value={m.id}>{m.fullName}</option>)}</select>
...
function WealthSummaryDashboard({ assets, baseCurrency, legacyPillars, liveRates }: { assets: any[]; baseCurrency: string; legacyPillars: { name: string; description: string }[]; liveRates: { [key: string]: number } }) {
                        ...
                        {editingId === asset.id ? (
                          <form action={async (fd) => { await updateAssetAction(asset.id, fd); setEditingId(null); }} className="w-full space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              <select name="rationale" defaultValue={asset.rationale} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded p-2 text-slate-900 dark:text-white text-sm cursor-pointer shadow-sm">
                                {legacyPillars.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
                              </select>
                            </div>
                            <div className="flex justify-end gap-1.5"><button type="button" onClick={() => setEditingId(null)} className="px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded text-slate-600 dark:text-slate-300 text-xs shadow-sm"><X className="w-3.5 h-3.5" /></button><button type="submit" className="px-2 py-1 bg-emerald-700 rounded text-white text-xs shadow-sm"><Check className="w-3.5 h-3.5" /></button></div>
                          </form>
                        ) : (
                          <>
                            <div className="min-w-0 pr-2">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 w-full max-w-2xl shadow-xl max-h-[85vh] overflow-y-auto my-auto text-slate-800 dark:text-slate-200 text-sm space-y-4">
        <div className="flex justify-between items-center pb-3 border-b border-slate-200 dark:border-slate-800">
          <h2 className="text-base font-bold text-slate-900 dark:text-white">{titles[type]}</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 cursor-pointer"><X className="w-5 h-5" /></button>
        </div>
        {type === 'about' && (
        ...
        {type === 'faq' && (
          <div className="space-y-4 text-slate-600 dark:text-slate-300">
            <div><div className="font-bold text-slate-900 dark:text-white mb-1">Q: How are live currency exchange rates updated?</div><p className="text-slate-500 dark:text-slate-400">A: OmniWealth fetches real-time fiat exchange rates via Frankfurt API and crypto prices via CoinGecko.</p></div>
            <div><div className="font-bold text-slate-900 dark:text-white mb-1">Q: Is my document vault secure?</div><p className="text-slate-500 dark:text-slate-400">A: Yes, all uploaded files and sensitive statements are encrypted at rest using AES-256 encryption.</p></div>
          </div>
        )}
        {type === 'privacy' && (<div className="space-y-3 text-slate-600 dark:text-slate-300"><p>Your privacy is paramount. OmniWealth stores your data in encrypted database columns and secure cryptographic vaults.</p></div>)}
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800 gap-4">
        ...
        <button 
          onClick={onAddLiability} 
          className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2.5 bg-rose-700 hover:bg-rose-800 text-white font-semibold text-xs rounded-xl cursor-pointer shadow-sm transition shrink-0"
        >
          <Plus className="w-4 h-4" />
          <span>Add Liability</span>
        </button>
